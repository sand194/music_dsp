...
# find all .R and .Rmd files and count their lines
for file in $(find . -type f \( -name "*.R" -o -name "*.Rmd" \)); do
    local lines=$(wc -l < "$file")
    total_lines=$((total_lines + lines))
done

echo "Total lines of code in .R and .Rmd files: $total_lines"
```
...
top_artists_spotify <- artists_genre_comparison %>%
  ...
  top_n(55, wt = Sum_listeners_views_across)
...
top_artists_spotify %>%
  ggplot(
    ...
    geom_text(
      data = top_labels,
      # aes(label = Artist, size = log(Sum_listeners_views_across)),
      aes(label = Artist),
      vjust = 0.5, 
      hjust = 0.5,
      size = 2,
...
# TODO pokolorować ładnie ten wykres + zrobić go dla polski
